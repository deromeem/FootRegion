...
class FootregionModeltournoiMatchs extends JModelList
{
	...
	protected function _getListQuery()
	{
		...
		$query->select('t.id AS Tournoi_id')->join('LEFT', '#__footregion_tournois AS t ON t.id=m.tournois_id')->where('m.tournois_id='.substr(JUri::getInstance(),(strpos(JUri::getInstance(), 'id='))+3));
		...
		// compléter le filtre pour avoir les matchs concerné par le tournoi
		//$query->where('m.tournois_id=' . (int) $pk) 
